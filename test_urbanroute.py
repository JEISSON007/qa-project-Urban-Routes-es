...
# 1 Para configurar la dirección
...
# 2.Seleccionar la tarifa Comfort.
    ...
    def test_choose_comfort(self):
        rates_page = ubanroute_page.UrbanRoutesPage(self.driver)
        rates_page.click_comfort_option()

        validation = ubanroute_page.UrbanRoutesPage(self.driver).get_validation_comfort()
        assert validation == 'Manta y pañuelos'

# 3.Rellenar el número de teléfono.

    def test_set_number_phone(self):
        phone_page = ubanroute_page.UrbanRoutesPage(self.driver)
...
# 4.Agregar una tarjeta de crédito.
    ...
    def test_add_card(self):
        card_page = ubanroute_page.UrbanRoutesPage(self.driver)
...
# 5.Escribir un mensaje para el controlador.
    ...
    def test_add_message_driver(self):
        comments_page = ubanroute_page.UrbanRoutesPage(self.driver)
...
# 6.Pedir una manta y pañuelos.
    ...
    def test_add_details(self):
        details_page = ubanroute_page.UrbanRoutesPage(self.driver)
...
# 7.Pedir 2 helados.
    ...
    def test_add_ice(self):
        ice_page = ubanroute_page.UrbanRoutesPage(self.driver)
        ice_page.add_two_icecream()

        element_count = ubanroute_page.UrbanRoutesPage(self.driver).get_ice_cream()
        assert element_count == "2"

# 8.Aparece el modal para buscar un taxi.

    def test_take_taxi(self):
        taxi_page = ubanroute_page.UrbanRoutesPage(self.driver)
        taxi_page.take_taxi()

        element_text = ubanroute_page.UrbanRoutesPage(self.driver).get_take_taxi()
...
# 9.Esperar a que aparezca la información del conductor en el modal.
    ...
    def test_wait_for_info(self):
        info_page = ubanroute_page.UrbanRoutesPage(self.driver)
        info_page.wait_for_load_information()

        driver_information = ubanroute_page.UrbanRoutesPage(self.driver).get_wait_driver_information()
        assert driver_information.is_displayed()

        travel_information = ubanroute_page.UrbanRoutesPage(self.driver).get_wait_travel_information()
